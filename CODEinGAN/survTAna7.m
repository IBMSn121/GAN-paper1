function survTAna7(sOSBC7)
pdd=4; fsTtF=10;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%ALL - TARGET
subplot(7,pdd,[1 1+pdd 1+pdd*2]);
Tobs=sOSBC7(:,1); Delta=sOSBC7(:,2); Icensor=sOSBC7(:,3); WCovX=sOSBC7(:,4:6);
bphreg = coxphfit(WCovX, Tobs, 'Censoring', Icensor); ScorePH=WCovX*bphreg;
qcp=0.66; SCTP=quantile(ScorePH,qcp); 
...
disp(['(10*) Trio''s prognosis for ALL - TARGET']); pvLR=logrankLF(BHH', BLL'); disp(['   ']); disp(['   ']);
...
LV=legend([HRG LRG PN],{'high risk (n = 70)', 'low risk (n = 137)', 'p = 2.05E-05'}, 'Location', 'southwest'); LV=legend('boxoff'); set(LV,'FontSize',11); set(gcf,'Color',[1,1,1]); 
axis tight; ylim([0 1.05]); xlim([0 60]); ylabel(['Survival  Rate']); xlabel(['Months']);
title(['TARGET dataset - ALL'],'FontSize',fsTt, 'FontWeight', 'bold');  
