function survTAna4(sOSBC4)
pdd=4; fsTtF=10;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%CLL - GSE22762
subplot(7,pdd,[1 1+pdd 1+pdd*2]);
Tobs=sOSBC4(:,1); Delta=sOSBC4(:,2); Icensor=sOSBC4(:,3); WCovX=sOSBC4(:,4:6);
bphreg = coxphfit(WCovX, Tobs, 'Censoring', Icensor); ScorePH=WCovX*bphreg;
qcp=0.15; SCTP=quantile(ScorePH,qcp); 
...
disp(['(7*) Trio''s prognosis for CLL - GSE22762']); pvLR=logrankLF(BHH', BLL'); disp(['   ']); disp(['   ']);
...
LV=legend([HRG LRG PN],{'high risk (n = 91)', 'low risk (n = 16)', 'p = 0.014'}, 'Location', 'southwest');
...
title(['GSE22762 - CLL'],'FontSize',fsTt, 'FontWeight', 'bold');   
